...
export class RingBufferWriter<BufferClassType extends ArrayBufferClassTypes> {
  ...
  #storage: InstanceType<BufferClassType>
  ...
  availableWrite() {
    const readPosition = Atomics.load(this.#readPointer, 0);
    const writePosition = Atomics.load(this.#writePointer, 0);
    return this.#availableWrite(readPosition, writePosition);
  }

  #availableWrite(readPosition: number, writePosition: number) {
  ...
  #copy(input: InstanceType<BufferClassType>, offsetInput: number, output: InstanceType<BufferClassType>, offsetOutput: number, size: number) {
