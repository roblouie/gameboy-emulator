...
const instructionCacheSize = 5000;
...
export function updateInstructionCache(instruction: string, programCounterVal: number, af: number, bc: number, de: number, hl: number, sp: number) {
  console.log(`${programCounterVal.toString(16)}: ${instruction} --AF: ${af.toString(16).padStart(4, '0')} --BC: ${bc.toString(16).padStart(4, '0')} --DE: ${de.toString(16).padStart(4, '0')} --HL: ${hl.toString(16).padStart(4, '0')} --SP: ${sp.toString(16)}`);
